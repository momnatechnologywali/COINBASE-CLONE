<?php
// buy.php - Buy crypto with market/limit orders
session_start();
include 'db.php';
...
// Handle order placement (dummy execution for market, pending for limit)
if ($_POST && isset($_POST['place_order'])) {
    ...
    // Fetch current price for market order USD value
    $current_price = 60000;  // Dummy, in real use API
    ...
    if ($type === 'market') {
        // Simulate fill: deduct from fiat (assume user has USD balance, dummy)
        // Add to wallet
        ...
        $stmt = $pdo->prepare("INSERT INTO transactions (user_id, type, currency, amount, usd_value) VALUES (?, 'buy', ?, ?, ?)");
        $stmt->execute([$user_id, $currency, $amount, $usd_value]);
    }
    // For limit, just insert and assume later execution
}
 
// Fetch current prices
$current_prices = ['BTC' => 60000, 'ETH' => 3000];  // Dummy
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn { width: 100%; background: #4CAF50; color: white; padding: 1rem; border: none; border-radius: 5px; cursor: pointer; font-size: 1.1rem; margin-top: 1rem; }
        .btn:hover { background: #45a049; }
        .price-display { text-align: center; font-size: 1.2rem; margin: 1rem 0; color: #00ff88; }
...
<body>
    <div class="order-form">
        ...
        <form method="POST">
            ...
            <input type="number" name="amount" placeholder="Amount in USD" step="0.01" required>
